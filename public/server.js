...
/* ---------------- contacts / inbox ---------------- */
// Counts for the inbox filter pane.
app.get('/api/inbox/counts', auth, wrap(async (req, res) => {
  const cid = req.user.company_id;
  const [open, closed, awaiting, unread, mine, unassigned, total] = await Promise.all([
    row(`SELECT COUNT(*) AS n FROM contacts WHERE company_id = ? AND status = 'open'`, cid),
    row(`SELECT COUNT(*) AS n FROM contacts WHERE company_id = ? AND status = 'closed'`, cid),
    row(`SELECT COUNT(*) AS n FROM contacts WHERE company_id = ? AND status = 'open' AND last_direction = 'in'`, cid),
    row('SELECT COUNT(*) AS n FROM contacts WHERE company_id = ? AND unread > 0', cid),
    row('SELECT COUNT(*) AS n FROM contacts WHERE company_id = ? AND owner_user_id = ?', cid, req.user.id),
    row('SELECT COUNT(*) AS n FROM contacts WHERE company_id = ? AND owner_user_id IS NULL', cid),
    row('SELECT COUNT(*) AS n FROM contacts WHERE company_id = ?', cid),
  ]);
  res.json({
    open: num(open.n), closed: num(closed.n), awaiting: num(awaiting.n),
    unread: num(unread.n), mine: num(mine.n), unassigned: num(unassigned.n), all: num(total.n),
  });
}));

app.get('/api/contacts', auth, wrap(async (req, res) => {
  const cid = req.user.company_id;
  const { view, owner, tag, q, from, to, limit } = req.query;
  const where = ['company_id = ?'];
  const params = [cid];

  if (view === 'open') where.push(`status = 'open'`);
  else if (view === 'closed') where.push(`status = 'closed'`);
  else if (view === 'awaiting') where.push(`status = 'open' AND last_direction = 'in'`);
  else if (view === 'unread') where.push('unread > 0');
  else if (view === 'mine') { where.push('owner_user_id = ?'); params.push(req.user.id); }
  else if (view === 'unassigned') where.push('owner_user_id IS NULL');

  if (owner === 'none') where.push('owner_user_id IS NULL');
  else if (owner) { where.push('owner_user_id = ?'); params.push(Number(owner)); }

  if (tag) { where.push('tags LIKE ?'); params.push('%"' + String(tag).replace(/"/g, '') + '"%'); }

  if (q) {
    where.push('(LOWER(name) LIKE ? OR phone LIKE ? OR LOWER(company) LIKE ?)');
    const like = '%' + String(q).toLowerCase() + '%';
    params.push(like, '%' + q + '%', like);
  }
  if (from) { where.push('COALESCE(last_message_at, created_at) >= ?'); params.push(from); }
  if (to) { where.push('COALESCE(last_message_at, created_at) <= ?'); params.push(to); }

  const cap = Math.min(Number(limit) || 500, 1000);
  const list = await all(
    `SELECT * FROM contacts WHERE ${where.join(' AND ')}
     ORDER BY COALESCE(last_message_at, created_at) DESC LIMIT ${cap}`, ...params);
  res.json(list.map((c) => ({ ...c, tags: JSON.parse(c.tags || '[]') })));
}));

// Resolve / reopen a conversation
app.post('/api/contacts/:id/status', auth, wrap(async (req, res) => {
  const c = await row('SELECT * FROM contacts WHERE id = ? AND company_id = ?', req.params.id, req.user.company_id);
  if (!c) return res.status(404).json({ error: 'Conversation not found' });
  const status = req.body?.status === 'closed' ? 'closed' : 'open';
  await run(
    `UPDATE contacts SET status = ?, closed_at = ${status === 'closed' ? "(NOW() AT TIME ZONE 'utc')" : 'NULL'} WHERE id = ?`,
    status, c.id);
  await run(
    'INSERT INTO messages (company_id, contact_id, direction, body, sent_by_user_id, status) VALUES (?,?,?,?,?,?)',
    req.user.company_id, c.id, 'note',
    status === 'closed' ? 'Conversation resolved' : 'Conversation reopened', req.user.id, 'sent');
...
app.post('/api/messages/:contactId', auth, wrap(async (req, res) => {
  ...
  await run(
    `UPDATE contacts SET last_message_at = (NOW() AT TIME ZONE 'utc'), last_direction = 'out' WHERE id = ?`, c.id);
  res.json({ ok: true, demo: result.demo });
}));

/* ---------------- quick replies ---------------- */
app.get('/api/quick-replies', auth, wrap(async (req, res) => {
  res.json(await all('SELECT * FROM quick_replies WHERE company_id = ? ORDER BY uses DESC, title', req.user.company_id));
}));

app.post('/api/quick-replies', auth, wrap(async (req, res) => {
  const { title, body, shortcut = '' } = req.body || {};
  if (!title || !body) return res.status(400).json({ error: 'Title and message are required' });
  const created = await run(
    'INSERT INTO quick_replies (company_id, title, body, shortcut, created_by) VALUES (?,?,?,?,?) RETURNING id',
    req.user.company_id, title, body, String(shortcut).replace(/^\//, '').toLowerCase(), req.user.id);
  res.json(await row('SELECT * FROM quick_replies WHERE id = ?', created.id));
}));

app.patch('/api/quick-replies/:id', auth, wrap(async (req, res) => {
  const qr = await row('SELECT * FROM quick_replies WHERE id = ? AND company_id = ?', req.params.id, req.user.company_id);
  if (!qr) return res.status(404).json({ error: 'Quick reply not found' });
  const { title, body, shortcut, used } = req.body || {};
  if (used) await run('UPDATE quick_replies SET uses = uses + 1 WHERE id = ?', qr.id);
  else {
    await run('UPDATE quick_replies SET title=?, body=?, shortcut=? WHERE id=?',
      title ?? qr.title, body ?? qr.body,
      shortcut !== undefined ? String(shortcut).replace(/^\//, '').toLowerCase() : qr.shortcut, qr.id);
  }
  res.json({ ok: true });
}));

app.delete('/api/quick-replies/:id', auth, wrap(async (req, res) => {
  await run('DELETE FROM quick_replies WHERE id = ? AND company_id = ?', req.params.id, req.user.company_id);
  res.json({ ok: true });
}));
...
async function runBots(company, contact, text, isNewContact, { send }) {
  ...
  // deliberately does NOT clear last_direction: an automated reply doesn't mean
  // a human has answered, so the chat stays in "Awaiting reply".
...
async function handleWebhook(companyId, payload) {
  ...
  for (const msg of entry?.messages || []) {
    ...
    // an inbound message reopens a resolved conversation
    await run(`UPDATE contacts SET unread = unread + 1, last_message_at = (NOW() AT TIME ZONE 'utc'),
               last_direction = 'in', status = 'open', closed_at = NULL WHERE id = ?`, contact.id);
